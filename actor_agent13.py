...
class ActorAgent3(Agent):
    def __init__(self,sess,node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        self.node_act_probs3, self.job_act_probs3 = self.actor_network3(
        ...
        self.node_entropy = tf.reduce_sum(tf.multiply(
            self.node_act_probs3, tf.log(self.node_act_probs3 + self.eps)))

        # prob on each job
        self.prob_each_job = tf.reshape(
            tf.sparse_tensor_dense_matmul(self.gsn.summ_mats[0],
                                          tf.reshape(self.node_act_probs3, [-1, 1])),
            [tf.shape(self.node_act_probs3)[0], -1])

        # job entropy
        self.job_entropy = \
            tf.reduce_sum(tf.multiply(self.prob_each_job,tf.reduce_sum(tf.multiply(self.job_act_probs3,
            tf.log(self.job_act_probs3 + self.eps)),
            reduction_indices=2)))
        ...
        # normalize entropy
        self.entropy_loss /= \
            (tf.log(tf.cast(tf.shape(self.node_act_probs3)[1], tf.float32)) +
    ...
    def actor_network3(self, node_inputs, gcn_outputs, job_inputs,
    ...
    def load_model3(self, model_path):
        variables_to_restore = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="actor_agent3")
        saver = tf.train.Saver(variables_to_restore)
        saver.restore(self.sess, model_path)

    def save_model3(self, file_path):
    ...
    def predict(self, node_inputs, job_inputs,
            ...
            gcn_mats, gcn_masks, summ_mats,
            running_dags_mat, dag_summ_backward_map):
        return self.sess.run([self.node_act_probs3, self.job_act_probs3],
            feed_dict={i: d for i, d in zip(
                [self.node_inputs] + [self.job_inputs] + \
                [self.node_valid_mask] + [self.job_valid_mask] + \
                self.gcn.adj_mats + self.gcn.masks + self.gsn.summ_mats + \
