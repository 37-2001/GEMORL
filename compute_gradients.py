...
def compute_actor_gradients(actor_agent, exp, batch_adv, entropy_weight):
    ...
    for b in range(len(batch_points) - 1):
        ...
        # compute gradient
        act_gradients, loss,node_entropy, job_entropy, selected_job_p,selected_node_p = actor_agent.get_gradients(
            node_inputs, job_inputs,
            node_valid_mask, job_valid_mask,
            extended_gcn_mats, extended_gcn_masks,
            extended_summ_mats, extended_running_dag_mats,
            summ_backward_map, node_act_vec,job_act_vec, adv, entropy_weight)

        all_gradients.append(act_gradients)
        all_loss[0].append(loss[0])  # adv_loss
        all_loss[1].append(loss[1])  # entropy_loss
    ...
    return gradients, all_loss, node_entropy, job_entropy

def clip_actor_gradients(actor_gradients, clip_norm=0.7):
    clipped_gradients_all = []
    for grad_list in actor_gradients:  # actor_gradients 可以是 [actor_gradient1, actor_gradient2, actor_gradient3]
        # 先处理 IndexedSlices，去掉 None
        grads_to_clip = [tf.convert_to_tensor(g) if isinstance(g, tf.IndexedSlices) else g
                         for g in grad_list if g is not None]
        # 使用全局裁剪
        clipped_grads, global_norm = tf.clip_by_global_norm(grads_to_clip, clip_norm)
        # 保持 None 的位置
        clipped_grads_full = []
        j = 0
        for g in grad_list:
            if g is None:
                clipped_grads_full.append(None)
            else:
                clipped_grads_full.append(clipped_grads[j])
                j += 1
        clipped_gradients_all.append(clipped_grads_full)
    return clipped_gradients_all
